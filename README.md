...
10. [セキュリティ](#セキュリティに関する重要な注意点)
   ...
   - [リスクと対策](#具体的なリスクと対策)
11. [Githubを使ってサイト作成](#Githubを使ってサイト作成)
12. [参考資料](#参考資料)
...
IPアドレスの見分け方：
- 有線LAN接続：`eth0`インターフェースの`inet`の後に表示されるIPアドレス
- 無線LAN接続：`wlan0`インターフェースの`inet`の後に表示されるIPアドレス

プライベートIPアドレスの範囲：
プライベートIPアドレスとして使用できる範囲は以下の3種類のみです：
...
注意点：
- これらの範囲以外のIPアドレスは、プライベートIPアドレスとして使用できません
- プライベートIPアドレスは、インターネット上で直接ルーティングされません
- 同じネットワーク内で重複するIPアドレスを使用することはできません
